...
describe('exportToSVG', () => {
  ...
  it('renders multiple parts', () => {
    const svg = exportToSVG([makePlaced('a', 10, 10, 0, 0), makePlaced('b', 20, 15, 15, 0)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
  ...
  it('renders rotated parts with different coordinates than unrotated', () => {
    const rotated = exportToSVG([makePlaced('a', 10, 20, 0, 0, Math.PI / 2)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
    const unrotated = exportToSVG([makePlaced('a', 10, 20, 0, 0, 0)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
  ...
  it('renders parts with multiple polygons', () => {
    ...
    const svg = exportToSVG([{ part, x: 0, y: 0, rotation: 0 }], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
    expect((svg.match(/<path/g) ?? []).length).toBe(2);
