...
describe('exportToLightBurn', () => {
  ...
  it('includes sheet boundary rect', () => {
    ...
    expect(xml).toContain('W="200"');
    expect(xml).toContain('H="150"');
  });
  ...
  it('renders placed parts as Path shapes', () => {
    ...
    expect(xml).toContain('<VertList>');
    expect(xml).toContain('<PrimList>');
  });

  it('includes vertex coordinates', () => {
    const xml = exportToLightBurn([makePlaced('a', 10, 10, 5, 5)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
    expect(xml).toContain('V5.0000 5.0000');
    expect(xml).toContain('V15.0000 5.0000');
  });
  ...
  it('uses line primitives', () => {
    ...
    expect(xml).toContain('L0 1');
    expect(xml).toContain('L3 0');
  });

  it('renders multiple parts', () => {
    const xml = exportToLightBurn([makePlaced('a', 10, 10, 0, 0), makePlaced('b', 20, 15, 15, 0)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
  ...
  it('renders rotated parts with different vertices than unrotated', () => {
    const rotated = exportToLightBurn([makePlaced('a', 10, 20, 0, 0, Math.PI / 2)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
    const unrotated = exportToLightBurn([makePlaced('a', 10, 20, 0, 0, 0)], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
  ...
  it('skips degenerate 2-point polygons', () => {
    ...
    const xml = exportToLightBurn([{ part, x: 0, y: 0, rotation: 0 }], {
      sheetWidth: 100,
      sheetHeight: 100,
    });
