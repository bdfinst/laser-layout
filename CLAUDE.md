...
3. **Engine** (`nesting/engine.ts`) orchestrates multi-sheet nesting. `nestPartsIterative()` is a generator that fills one sheet at a time, yielding progress per GA generation. Parts that don't fit on the current sheet overflow to the next. `makeOptimizerConfig()` maps `NestingConfig` (incl. the optional `stallWindow`/`stallEpsilon`/`maxGenerations`) to the optimizer config, defaulting `maxGenerations = max(generations * 3, 120)`, `stallWindow = 15`, `stallEpsilon = 0.005`.
   ...
   **Heterogeneous sheet sizes (`docs/specs/heterogeneous-sheet-sizes.md`).** `NestingConfig.sheets: MaterialSheet[]` lists the available sheet sizes (the legacy single `sheet` still works — `availableSheets(config)` in `geometry/types.ts` normalizes one or the other into a list; `sheets` wins; an empty list throws). Each `MaterialSheet` has an optional `maxCount` (omitted ⇒ unlimited supply). For each opened sheet the engine calls `selectSheetForNextOpen()` to pick — among the in-supply sizes that can hold the largest remaining fittable part — the size minimizing committed area (a cheap GA-free area-fit heuristic; the GA then runs once on the chosen size, never per-size). `SupplyPool` (`nesting/supply-pool.ts`, identity-keyed) tracks remaining counts and caps how many of each size open; when supply is exhausted the remaining parts (incl. `required`) go to `unplaced`. Each `SheetResult` carries its own `sheetWidth`/`sheetHeight` so export (`exportSheetsToSVG`/`exportSheetsToLightBurn`) and the layout preview size every sheet individually. The result comparator (`isBetterResult`) ranks feasibility → least committed area (Σ per-sheet W×H) → fewer sheets → least strip area — a no-op for a single uniform size. The UI exposes an editable list of sizes with per-size "Max sheets" in `MaterialSettings.svelte`.
