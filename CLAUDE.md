...
**NFP placement path (P3–P5), opt-in via `NestingConfig.useNfpPlacement` (default `false`).** When enabled, the **exact** collision phase of `bottomLeftFill` augments its anchors with full NFP touching seats (P3), selects among validated candidates by resulting strip height rather than pure bottom-left (P4, `resultingStrip`), and replaces the O(edgesA·edgesB) true-shape collision with a signed NFP-clearance test (`nfpClearance`, P5) that also expresses kerf as a clearance threshold. The cache is created per sheet in the optimizer and shared across exact-phase evaluations; the fast bbox search and all non-opt-in callers are byte-for-byte unchanged. **It is off by default because, benched, it is ~3–4× slower with neutral density and does not yet reach the lego-shelves one-sheet KPI** — the one-sheet target needs the GA to search _with_ NFP (not just the short exact tail), which remains the open tuning work. See the `lego-shelves[nfp=0|1]` rows in `bench/nesting-compaction.bench.ts`.
