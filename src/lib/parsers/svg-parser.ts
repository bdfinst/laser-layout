  ...
  for (const { command, args } of tokens) {
    switch (command) {
      ...
      case 'C': {
        ...
        currentPoly.push(...pts);
        lastCp = { x: args[2], y: args[3] };
        cx = args[4];
        cy = args[5];
        break;
      }
      case 'c': {
        ...
        lastCp = { x: cx + args[2], y: cy + args[3] };
        currentPoly.push(...pts);
        cx += args[4];
        cy += args[5];
        break;
      ...
      case 's': {
        ...
        lastCp = { x: cx + args[0], y: cy + args[1] };
        currentPoly.push(...pts);
        cx += args[2];
        cy += args[3];
        break;
...
export function parseSVG(svgString: string): Part[] {
  if (svgString.length > MAX_FILE_SIZE) {
    throw new Error(
      `SVG file too large (${(svgString.length / 1024 / 1024).toFixed(1)} MB). Maximum is 10 MB.`,
    );
  }
